...
export class PostulacionListaPorPlazaLaboralUsecase implements UseCase<{id: number}, ResponseModel<PostulacionModel[]>> {
     ...
     * Invoca el método `listaPorPlazaLaboral` del PostulacionGateway para obtener las postulaciones de un postulante específico.
     *
     * @param {number} params.id - Identificador único de la plaza laboral cuyas postulaciones se desean obtener.
    ...
    perform(params: { id: number; }): Observable<ResponseModel<PostulacionModel[]>> {
       return this._postulacionGateway.listaPorPlazaLaboral(params);
    }
}
