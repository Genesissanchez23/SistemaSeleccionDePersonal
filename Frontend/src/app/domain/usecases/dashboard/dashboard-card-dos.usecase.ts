...
/**
  * Caso de uso concreto para obtener los datos de la segunda tarjeta del dashboard.
  * Implementa la interfaz UseCase para definir la operación `perform`.
*/
@Injectable({
  providedIn: 'root'
})
export class DashboardDosUsecase implements UseCase<void, CategoryData[]> {
  ...
  */
  ...
  /**
    ...
    * Invoca el método `getCardDos` del DashboardGateway para obtener los datos de la segunda tarjeta del dashboard.
    *
    * @returns {Observable<CategoryData[]>} Observable que emite una respuesta con los datos de la segunda tarjeta del dashboard.
  */
  perform(): Observable<CategoryData[]> {
    return this._dashboardGateway.getCardDos();
  }
}
