...
/**
  * Caso de uso concreto para obtener los datos de la quinta tarjeta del dashboard.
  * Implementa la interfaz UseCase para definir la operación `perform`.
*/
@Injectable({
  providedIn: 'root'
})
export class DashboardCincoUsecase implements UseCase<void, DataItems[]> {
  ...
  */
  ...
  /**
    ...
    * Invoca el método `getCardCinco` del DashboardGateway para obtener los datos de la quinta tarjeta del dashboard.
    *
    * @returns {Observable<DataItems[]>} Observable que emite una respuesta con los datos de la quinta tarjeta del dashboard.
  */
  perform(): Observable<DataItems[]> {
    return this._dashboardGateway.getCardCinco();
  }
}
